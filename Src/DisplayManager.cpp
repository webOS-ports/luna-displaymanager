    ...
    , m_powerKeyPressEventScheduled(false)
{
    ...
    result = LSRegisterServerStatusEx(m_service, "com.palm.systemmanager", DisplayManager::authServiceNotification, this, NULL, &lserror);
    if (!result)
    {
        LSErrorPrint (&lserror, stderr);
        LSErrorFree (&lserror);
...
const std::string& DisplayManager::puckId() const
{
    return m_puckId;
}
...
{
    // Fed by the getDeviceLockMode subscription (see cbDeviceLockModeChanged):
    // in-process this used to ask Security and EASPolicyManager directly, but
    // those live in luna-authmanager now. "pending" policy state is treated
    // as requiring a passcode even before one is set - same fail-toward-
    // locked bias the old policyPending()/passwordRequired() check had.
    return m_unlockRequiresPasscode;
}

bool DisplayManager::cbDeviceLockModeChanged(LSHandle* handle, LSMessage* message, void* ctx)
{
    DisplayManager* dm = (DisplayManager*) ctx;
